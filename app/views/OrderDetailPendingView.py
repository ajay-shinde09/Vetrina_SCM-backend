...
#                     otp.batch,  -- Added batch from order_table_product
#                     v.name AS customer_name,  -- Customer name from vetzone
#                     po.payment_method,  -- Payment method from placeorder
#                     otp.product_id,  -- Added product_id for stock calculation
...
#                     cursor.execute('''
#                         SELECT 
#                             SUM(otp.quantity * otp.rate) / 3 AS avgSale
#                         FROM 
#                             order_table_product otp
#                         WHERE 
#                             otp.user_id = %s
#                             AND otp.created_at >= DATE_SUB(CURDATE(), INTERVAL 3 MONTH)
#                             AND otp.product_id = %s;
#                     ''', [created_by, product_id])
