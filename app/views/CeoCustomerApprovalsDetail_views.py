...
from app.serializers.CeoCustomerApprovalsDetail_serializers import CeoCustomerApprovalDetailSerializer
import os
...
class CeoCustomerApprovalDetailView(APIView):
...
'''
    ...
    "credit_limit": meta_data.get('credit_limit', None)
        ...
        #         file_img = f"{settings.MEDIA_URL}customer_kyc_document/{file_name}"
        #     else:
        ...
        serializer = CeoCustomerApprovalDetailSerializer(data=data)
    ...
    def put(self, request, customer_id):
        try:
            ...
            with connection.cursor() as cursor:
                # Fetch the customer email from the database
                cursor.execute("SELECT email FROM customer WHERE customer_id = %s", [customer_id])
                customer = cursor.fetchone()
                if not customer:
                    return Response({'status': 'error', 'message': 'Customer not found'}, status=404)
                
                customer_email = customer[0]
                # Update approval_status in the customer table to 'y' (approved)
                cursor.execute("""
                    UPDATE customer
            ...
            subject = 'Your Customer Approval'
            message = (
                f"Dear Customer,\n\n"
                f"Your account has been successfully approved. Your login details are as follows:\n\n"
                f"Email: {customer_email}\n"
                f"Password: 123456\n\n"
                f"Please log in and change your password immediately for security purposes.\n\n"
                f"Best regards,\nYour Company"
            )
